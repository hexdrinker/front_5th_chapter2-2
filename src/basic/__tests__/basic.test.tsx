...
describe('basic > ', () => {
  ...
  describe('cartUtils', () => {
    ...
    describe('calculateCartTotal', () => {
      ...
      test('쿠폰 없이 총액을 올바르게 계산해야 합니다.', () => {
        const { totalBeforeDiscount, defaultAfterDiscount } =
          cartUtils.calculateCartTotal(cart)
        ...
        expect(totalBeforeDiscount).toBe(400)
        expect(defaultAfterDiscount).toBe(380)
        expect(totalAfterDiscount).toBe(380)
      ...
      test('금액쿠폰을 올바르게 적용해야 합니다.', () => {
        ...
        expect(totalAfterDiscount).toBe(330)
        expect(totalDiscount).toBe(70)
      })
      ...
      test('퍼센트 쿠폰을 올바르게 적용해야 합니다', () => {
        ...
        expect(totalAfterDiscount).toBe(342)
        expect(totalDiscount).toBe(58)
      })
    })
  ...
  describe('useCart > ', () => {
    ...
    test('합계를 정확하게 계산해야 합니다', () => {
      ...
      const { totalBeforeDiscount, defaultAfterDiscount } =
        cartResult.current.calculateTotal()

      const { totalAfterDiscount } = useDiscountCalculator({
        defaultAfterDiscount,
        selectedCoupon: couponResult.current.selectedCoupon,
        selectedTier: null,
      })
      const totalDiscount = totalBeforeDiscount - totalAfterDiscount

      expect(totalBeforeDiscount).toBe(200)
      expect(defaultAfterDiscount).toBe(200)
